...
import { calculateTeamScore, determineWinner, saveGameResult, updatePlayerRating } from '../utils/gameutils.js';
...
router.post('/game/:opponent', async (req, res) => {
    ...
    try {
        ...
        await updatePlayerRating(winnerId, loserId);
        ...
        const matchResultMessage = winnerId === userId
...
router.post('/matchmaking', async (req, res) => {
    try {
        ...
        if (matchOpponents.length === 0) {
            return res.status(404).json({ error: '매칭 가능한 유저가 없습니다.' });
        }
        // 랜덤으로 상대방 선택
        ...
        const { result, winnerId, loserId } = await startGame(userId, selectedOpponent.id);
        ...
        res.json({ result, "상대아이디":selectedOpponent.id,"상대닉네임": selectedOpponent.nickname, 
            ...matchResultMessage, "나의 점수" : updatedUserRaiting.rating});
    } catch (error) {
...
async function startGame(userId, opponentId) {
    try {
        ...
        // 경기 결과 저장
        await saveGameResult(userId, opponentId, aScore, bScore);
